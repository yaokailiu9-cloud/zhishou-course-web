...
- 已核实的镜像同步管理入口：`https://github.com/zhouliujunteng/zhishou-course-web/actions/workflows/sync-fork.yml`，工作流名为 `Sync Fork 上游代码`，支持 `workflow_dispatch`。这不是源仓库的 Actions 页面；不能因源仓库没有工作流就判定没有同步入口。
- 注意登录身份：本机 `gh` 当前登录 `yaokailiu9-cloud`，它的协作仓库列表可能为空；Chrome 已有 `zhouliujunteng` 登录会话，可从 GitHub 账户切换器进入上述私有镜像仓库。通过正常 UI 使用已有授权，不复制浏览器 Cookie 或令牌，也不擅自新增权限。浏览器扩展连接超时时，可用原生 Chrome 可访问性界面读取和操作。
