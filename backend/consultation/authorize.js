...
var actor = {accountId: accountId ? id(accountId) : null, providerId: null, serviceKind: "", canReply: false, canAccept: false, agentId: null, canInvite: false};
if (actor.accountId) {
  ...
  if (provider && provider.service_kind === "AGENT") {
    actor.agentId = provider.id;
    actor.serviceKind = "AGENT";
    actor.canInvite = true;
  } else if (provider) {
    actor.providerId = provider.id;
    actor.serviceKind = provider.service_kind || "STAFF";
    actor.canReply = provider.can_reply === true;
    actor.canAccept = provider.can_accept_order === true;
    actor.canInvite = actor.canAccept;
  }
}
