...
function(scope){with(scope){try{return (item.subtitle === '《答案库》系列课程' ? item.title : item.badge)}catch(_){return undefined}}},
...
function(scope){with(scope){try{return (featuredCourse.subtitle === '《答案库》系列课程' ? featuredCourse.title : featuredCourse.badge)}catch(_){return undefined}}},
...
function(scope){with(scope){try{return (enrollment && enrollment.status === 'REGISTERED' ? '查看我的报名' : '报名')}catch(_){return undefined}}},
function(scope){with(scope){try{return (busy || loading || !classInfo.canEnroll)}catch(_){return undefined}}},
function(scope){with(scope){try{return (classInfo.canEnroll ? '确认报名' : classInfo.closedReason)}catch(_){return undefined}}},
...
function(scope){with(scope){try{return (busy ? '正在保存课程…' : status === 'PUBLISHED' ? '保存已发布课程' : '发布公开课')}catch(_){return undefined}}},
...
Page({
 ...
 onShareAppMessage(){return {title:'公开课 · 一起学习如何更好地沟通',path:'/pages/public-class/public-class'};}
});
