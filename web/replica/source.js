...
function(scope){with(scope){try{return (records.length)}catch(_){return undefined}}},
function(scope){with(scope){try{return (records)}catch(_){return undefined}}},
...
function(scope){with(scope){try{return (!recordsLoading && !records.length)}catch(_){return undefined}}},
function(scope){with(scope){try{return (recordsLoading)}catch(_){return undefined}}},
function(scope){with(scope){try{return (recordsLoading ? '正在加载…' : '查看更多问卷')}catch(_){return undefined}}},
function(scope){with(scope){try{return (recordsLoading && !records.length)}catch(_){return undefined}}},
...
Page({
 data:{loading:true,busy:false,error:'',offer:null,priceText:'19.90',paid:false,submitted:false,loggedIn:false,issues:[],form:empty()},
 ...
 async refresh(){
  ...
  try{
   ...
   this.enrollmentId=e&&e.id;this.setData({offer:r.offer,priceText:Number(r.offer&&r.offer.price||0).toFixed(2),issues:r.issues||[],paid:!!e,submitted:!!(e&&e.child_submitted_at),loggedIn:auth.isLoggedIn(),form});
  }catch(e){service.error(this,e);}finally{this.setData({loading:false});}
 },
