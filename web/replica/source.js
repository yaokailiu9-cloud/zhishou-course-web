...
function(scope){with(scope){try{return (mode === 'courses' ? '课程' : '知手')}catch(_){return undefined}}},
...
function(scope){with(scope){try{return (busy ? '正在确认支付…' : loggedIn ? '支付 ¥9.90，开始填写' : '微信登录并支付 ¥9.90')}catch(_){return undefined}}},
function(scope){with(scope){try{return (offer && paid)}catch(_){return undefined}}},
...
function(scope){with(scope){try{return (about ? '关于知手' : '帮助与支持')}catch(_){return undefined}}},
...
function classShare(c) {
  if (!c || !c.id || !['PUBLISHED','CLOSED'].includes(c.status))
    return {title:'知手 · 公开课',path:'/pages/public-class/public-class'};
  const share={title:c.title || '知手 · 公开课',path:'/pages/public-class-detail/public-class-detail?id='+encodeURIComponent(String(c.id))};
...
Page({
 ...
 onShareAppMessage(){return {title:'知手课程 · 一起学习如何更好地沟通',path:'/pages/public-class/public-class'};}
});
